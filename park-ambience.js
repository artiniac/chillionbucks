...
export function parkAmbience(){
 ...
 let menuOpen=false,unlocked=false,timer=null,bag=[],last=null;
 const allowed=()=>unlocked&&window.SFX?.on&&!document.hidden&&!menuOpen;
 ...
 document.addEventListener('cb:menu-open',()=>{menuOpen=true;sync();});document.addEventListener('cb:menu-close',()=>{menuOpen=false;sync();});
 return audio;
}
