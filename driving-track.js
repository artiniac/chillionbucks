...
PRESETS.family={name:'Leo’s Grand Prix',icon:'👨‍👦',width:20,smooth:true,points:PRESETS.academy.points.map(([x,y,z])=>[x*1.5,y,z*1.5])};
...
export function validate(points){
 ...
 if(points.some(p=>!Array.isArray(p)||p.length!==3||p.some(n=>!Number.isFinite(n))||Math.abs(p[0])>180||Math.abs(p[2])>180||p[1]<0||p[1]>18))return 'Keep your track inside the building field.';
