...
3. **Spend.** Every snap-together structure piece is free (Snap Blocks, Magnet Tiles, walls, roofs, floors, fences, roads, coaster track, river channels, slides, castle walls, garage decks), so a kid can always build a lazy river or a ten-story garage without saving. Pools, rides, characters, vehicles, and the rest cost bucks. Selling gives the full price back. Completing a build order pays a bill too, so building a whole lazy river both is free and earns.

The savings goals in the piggy game are real things with real prices: `goals.js` holds the defaults, and the "Grown-ups: add a real goal" form under the cards takes a store link, a name, a price, and the product photo's address (saved on that device only, `cb:goals`). When the wallet reaches a goal, the card gets a check, the kid makes a **certificate** (a PNG with their name, the amount, the goal, how many jobs and coins it took, and the date) to show a grown-up, and the grown-up store button opens the listing in a new tab.

Once a day, savings of $5 or more earn a little "baby money" (5%, capped at $5). Everything lives in this browser's localStorage (`cb:wallet`, `cb:world`, `cb:worlds`, `cb:work`, `cb:goals`, `cb:goalPicked`, `cb:kidName`, `cb:coins`); nothing is sent anywhere, and the site never reads anything from any other site.
