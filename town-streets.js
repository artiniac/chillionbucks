...
export function batchScene(root){root.updateMatrixWorld(true);const buckets=new Map();for(const o of [...root.children]){if(!o.isMesh||o.isInstancedMesh)continue;const geometry=o.geometry.clone().applyMatrix4(o.matrix),key=o.material;if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(geometry);o.geometry.dispose();root.remove(o);}for(const [material,geometries] of buckets){const geometry=mergeGeometries(geometries,false);geometries.forEach(g=>g.dispose());const m=new T.Mesh(geometry,material);m.castShadow=true;m.receiveShadow=true;root.add(m);}return root;}
