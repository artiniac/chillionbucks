...
const academy=curveFor(PRESETS.academy.points,true),planned=makeSpeedPlan(teachingLine(academy,16),academy.getLength(),19);assert.ok(Array.from({length:1024},(_,i)=>planned.at(i/1024)).some(p=>p.limit<Math.min(19,cornerSpeed(p.curvature))-.1),'Braking begins before tight bends, while still on the approach');
