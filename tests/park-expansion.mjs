...
const original={type:'pool',x:30,z:0};const s={items:[original],route:START_ROUTE.map(p=>[...p]),adventures:{ride:true}};const route=JSON.stringify(s.route);expandPark(s);assert.equal(s.items[0],original);assert.equal(s.items.filter(i=>i.x===30&&i.z===0).length,1);assert.equal(JSON.stringify(s.route),route);assert.equal(s.adventures.ride,true);const n=s.items.length;expandPark(s);assert.equal(s.items.length,n);
