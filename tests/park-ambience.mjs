...
document.hidden=false;events.get('visibilitychange')();events.get('cb:menu-open')();assert.equal(a.paused,true,'Chooser silences the park');events.get('cb:menu-close')();assert.equal(a.paused,false);window.SFX.on=false;events.get('cb:menu-open')();events.get('cb:menu-close')();assert.equal(a.paused,true,'Closing chooser preserves mute');
