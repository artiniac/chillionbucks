...
let count=0;
for(const d of TOWN_CATALOG.filter(d=>hasBlueprint(d.id))){const full=townModel(d.id),parts=townBlueprint(d.id),built=townConstructionModel(d.id,parts.length);assert.ok(parts.length>1);assert.deepEqual(parts.map(p=>p.id),BLUEPRINT_LAYOUT[d.id],d.id+' preserves saved assembly steps');assert.equal(vertices(full),vertices(built),d.id+' must assemble every visible triangle');const a=new T.Box3().setFromObject(full),b=new T.Box3().setFromObject(built);assert.ok(a.min.distanceTo(b.min)<1e-5&&a.max.distanceTo(b.max)<1e-5,d.id+' completed bounds');assert.equal(townConstructionModel(d.id,0).children.length,0);assert.equal(townConstructionModel(d.id,-10).children.length,0);assert.equal(townConstructionModel(d.id,9999).children.length,parts.length);assert.equal(townConstructionModel(d.id,1).children.length,1);assert.ok(vertices(full)<350000,d.id+' geometry budget');count++;}
