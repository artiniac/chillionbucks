...
3. **Kid-safe.** Audience is roughly ages 4 to 9. No accounts, no forms that collect anything, no tracking, no ads, no external links that leave the site except the YouTube creators' own videos. `localStorage` holds only on-device game state: sound on/off (`cb:sound`), the wallet (`cb:wallet`), and the Builder world (`cb:world`, which may include a downscaled room photo as a data URL). Nothing is ever uploaded; keep it that way. Any new stored key must be harmless if it leaks to nobody but the same browser.
...
- `build.html` + `builder.js` + `builder.css`: the Chillion Builder. Scenes are SVG strings in `SCENES`; hand-drawn animated stickers are SVG factory functions in `S` (each takes a unique id so clipPaths never collide); the catalog is `ITEMS` (`V(...)` for SVG stickers, `E(...)` for emoji). Prices are small whole dollars; free items exist so a world is never empty. Selling refunds the full price.
...
## Testing
No test framework. Before pushing: `node --check app.js builder.js sfx.js wallet.js`, `node verify-videos.mjs`, then open `index.html` and `build.html` in a browser at desktop and phone widths and click through every game once (drag a coin into the piggy, buy and drag a sticker, draw, save a picture).
