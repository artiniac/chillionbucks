...
export function makeCar(){
 ...
 result.ready=new GLTFLoader().loadAsync('./assets/cars/skyline-r34.glb').then(({scene})=>{
  ...
  const oldMaterials=new Set();result.body.traverse(o=>{if(o.isMesh){o.geometry.dispose();oldMaterials.add(o.material);}});for(const material of oldMaterials)if(material!==result.paint)material.dispose();result.body.clear();result.body.add(replacement);result.wheels.splice(0,result.wheels.length,...wheels);scene.traverse(o=>{if(o.isMesh)o.geometry.dispose();});return {meshes:buckets.size,wheels:wheels.length};
 });
 return result;
}
