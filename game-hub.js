...
(() => {
  const games = [
    ['drive','🏎️','Chill Drive','Follow the arrows. Find your flow.','driving.html'],
    ...
    ['classic','🏝️','Classic creations','Return to your original worlds.','classic.html'],
  ];
  ...
  function currentGame() {
    ...
    return games.find(g => g[4] === filename || g[4] === filename + location.hash)?.[0];
  }
  let recent = [];
  ...
  dialog.innerHTML='<div class="cb-menu-heading"><div><small>CHILLION BUCKS</small><h2 id="cbMenuTitle">What shall we play?</h2></div><button class="cb-menu-close" aria-label="Close game chooser">✕</button></div><div class="cb-game-grid"></div><div class="cb-menu-footer"><a href="index.html">⌂ Home</a><span>Your creations stay on this device.</span></div>';
  const grid=dialog.querySelector('.cb-game-grid');
  for(const [id,icon,title,description,url] of games){
    const a=document.createElement('a');a.href=url;a.dataset.game=id;
    a.innerHTML=`<span class="cb-game-icon" aria-hidden="true">${icon}</span><span><b>${title}</b><small>${description}</small></span><span class="cb-game-play" aria-hidden="true">▶</span>`;
  ...
  if(original){original.textContent='🎮 Games';original.classList.add('cb-games-link');original.setAttribute('aria-haspopup','dialog');original.addEventListener('click',openMenu);}
  else if(header){const b=document.createElement('button');b.className='cb-games-button';b.textContent='🎮 Games';b.setAttribute('aria-haspopup','dialog');b.onclick=openMenu;(isHome?header:(header.querySelector('nav')||header)).append(b);}
  if(isHome&&recent.length){
    ...
    for(const id of recent){const g=games.find(g=>g[0]===id);const a=document.createElement('a');a.href=g[4];a.textContent=g[1]+' '+g[2];section.append(a);}
