...
function material(color,type='plain'){const key=color+type;if(mats.has(key))return mats.get(key);if(typeof document.createElementNS==='function'&&['brick','roof','stone'].includes(type)){const m=scannedMaterial(type==='brick'?'red_brick':type==='stone'?'concrete_pavement':'roof_slates_02',[1,1],type==='brick'?'#dbcbc1':'#'+new T.Color(color).lerp(new T.Color('#ffffff'),.3).getHexString());m.normalScale.set(type==='stone'?.18:.65,type==='stone'?.18:.65);mats.set(key,m);return m;}const m=new T.MeshStandardMaterial({color,roughness:type==='glass'?.18:.78,metalness:type==='glass'?.45:.03});if(type==='glass'){m.roughness=.12;m.metalness=.7;m.envMapIntensity=1.4;}if(['stone','brick','wood','stucco','roof','clay'].includes(type)){const c=document.createElement('canvas');c.width=c.height=128;const ctx=c.getContext('2d');ctx.fillStyle='#dad4c7';ctx.fillRect(0,0,128,128);for(let i=0;i<2000;i++){ctx.fillStyle=i%2?'#ffffff15':'#00000012';ctx.fillRect((i*43.13)%128,(i*17.37)%128,1,type==='wood'?12:1);}if(type==='brick'){ctx.strokeStyle='#eee5d580';ctx.lineWidth=1;for(let y=0;y<128;y+=16){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(128,y);ctx.stroke();for(let x=(y/16)%2?16:0;x<128;x+=32){ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x,y+16);ctx.stroke();}}}if(type==='roof'){ctx.fillStyle='#9f9f96';ctx.fillRect(0,0,128,128);for(let row=0;row<10;row++){for(let col=-1;col<8;col++){const v=135+((row*43+col*31+311)%43);ctx.fillStyle='rgb('+v+','+v+','+(v-5)+')';ctx.fillRect(col*19+(row%2)*9,row*13,18,11);ctx.fillStyle='#00000028';ctx.fillRect(col*19+(row%2)*9,row*13+10,18,2);}}}if(type==='clay'){for(let px=0;px<128;px++){const v=130+Math.round(Math.sin(px/128*Math.PI)*70);ctx.fillStyle='rgb('+v+','+(v-12)+','+(v-27)+')';ctx.fillRect(px,0,1,128);}ctx.fillStyle='#55432c66';ctx.fillRect(0,121,128,7);}const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;tex.wrapS=tex.wrapT=T.RepeatWrapping;m.map=tex;m.bumpMap=tex;m.bumpScale=type==='stucco'?.015:.035;tex.anisotropy=8;}if(type==='clay'){m.side=T.DoubleSide;m.roughness=.93;m.bumpScale=.012;}m.userData.shared=true;mats.set(key,m);return m;}
...
function estate(g,id){const d=ESTATE_STYLES[id],spanish=['spanish','mission','hacienda'].includes(d.form),ranch=d.form==='ranch'||d.form==='hacienda',h=ranch?1.45:2.75;
 ...
 else if(spanish)clayRoof(g,0,h,-.65,5.1,2.9,.72,d.roof);else gable(g,0,h,-.65,5.1,2.9,ranch?.6:1.2,d.roof);
 ...
 if(spanish){
  for(const x of [-2.38,2.38]){const pier=box(g,d.wall,x,h/2,.67,.22,h,.22,'stucco');pier.userData.blueprintStage='walls';}
  ...
  if(d.tower){cylinder(g,d.wall,1.95,1.8,-.6,.65,3.6);const cone=new T.Mesh(new T.ConeGeometry(.76,.56,32),material(d.roof,'clay'));const uv=cone.geometry.attributes.uv;for(let i=0;i<uv.count;i++)uv.setXY(i,uv.getX(i)*22,uv.getY(i)*4);uv.needsUpdate=true;cone.castShadow=true;cone.position.set(1.95,3.88,-.6);g.add(cone);}
...
function compact(root){root.updateMatrixWorld(true);const buckets=new Map();root.traverse(o=>{if(!o.isMesh)return;const geo=o.geometry.index?o.geometry.toNonIndexed():o.geometry.clone();geo.applyMatrix4(o.matrixWorld);const key=o.material.uuid+(o.userData.groundSurface?'ground':'solid');let b=buckets.get(key);if(!b)buckets.set(key,b={material:o.material,ground:!!o.userData.groundSurface,p:[],n:[],uv:[]});for(const v of geo.attributes.position.array)b.p.push(v);for(const v of geo.attributes.normal.array)b.n.push(v);for(const v of geo.attributes.uv.array)b.uv.push(v);geo.dispose();o.geometry.dispose();});const result=new T.Group();for(const [m,b] of buckets){const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(b.p,3));geo.setAttribute('normal',new T.Float32BufferAttribute(b.n,3));geo.setAttribute('uv',new T.Float32BufferAttribute(b.uv,2));geo.userData.shared=true;const mesh=new T.Mesh(geo,b.material);mesh.castShadow=!b.ground;mesh.receiveShadow=true;mesh.userData.groundSurface=b.ground;result.add(mesh);}return result;}
function civic(g,id){
